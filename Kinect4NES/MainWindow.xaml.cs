...
namespace Kinect4NES
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void Reader_FrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            ...
            if (dataReceived)
            {
                // Make sure at least one body
                if (bodies.Length > 0)
                {
                    // Find the first engaged body
        ...
        private void CalcController(Body body)
        {
            IReadOnlyDictionary<JointType, Joint> joints = body.Joints;
            ...
            var mid = joints[JointType.SpineMid];
            var spineBase = joints[JointType.SpineBase];
            var knee = joints[JointType.KneeRight];
            var shoulder = joints[JointType.ShoulderRight];
            var leftWrist = joints[JointType.HandLeft];
            var rightWrist = joints[JointType.HandRight];
            var leftFoot = joints[JointType.FootLeft];
            var rightFoot = joints[JointType.FootRight];
            var leftAnkle = joints[JointType.AnkleLeft];
            var rightAnkle = joints[JointType.AnkleRight];
            var leftKnee = joints[JointType.KneeLeft];
            var rightKnee = joints[JointType.KneeRight];

            // What happens if joint is occluded?
            // if (hip == null || rightWrist == null) { return; }
            ...
            var runRight = ((leftFoot.Position.Z < rightFoot.Position.Z) && Math.Abs((leftFoot.Position.Y - rightFoot.Position.Y)) >= .10);
            var runLeft = ((rightFoot.Position.Z < leftFoot.Position.Z) && Math.Abs((leftFoot.Position.Y - rightFoot.Position.Y)) >= .10);
            var jump = ((spineBase.Position.Y - knee.Position.Y) < 0.28);

            /****************************************************************
             * Buttons
             ***************************************************************/
            // Left and right together is start
            var select = false;

            //Down and up together is select
            if (dpadDown && dpadUp)
            {
                dpadDown = false;
                dpadUp = false;
            ...
            var aButton = body.HandLeftState == HandState.Closed;
            var bButton = body.HandRightState == HandState.Closed;

            /****************************************************************
             * Set NES hardware interface pin states
             ***************************************************************/

            var dpadDownPin =  dpadDown ? DigitalPin.Low : DigitalPin.High;
            var dpadUpPin =  dpadUp ? DigitalPin.Low : DigitalPin.High;
            var dpadLeftPin =  dpadLeft ? DigitalPin.Low : DigitalPin.High;
            var dpadRightPin =  dpadRight ? DigitalPin.Low : DigitalPin.High;
            var selectPin =  select ? DigitalPin.Low : DigitalPin.High;
            var startPin =  start ? DigitalPin.Low : DigitalPin.High;
